let roles = ["Городничий", "Аммос Федорович", "Артемий Филиппович", "Лука Лукич", "Христиан Иванович"];

let text = `Городничий, попечитель богоугодных заведений, смотритель училищ, судья, частный пристав, лекарь, два квартальных.
...
roles.forEach(role => {
  ...
  phrases.forEach((line, i) => {
    if(line.indexOf(role) >= 0 && line.indexOf(role) <= role.length) {
      let content = line.slice(role.length + 1);
      temp += `${i + 1}) ${content}\n`;
    }
  })
