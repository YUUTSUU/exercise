...
function sortByFeedbacks(arr) {
  ...
  temp.forEach(item => {
    if(typeof item.price == "string") {
      ...
      item.price = parseInt(item.price.newUan.replace(/\D/g, ""))
    }

    item.ratingRevievs = parseInt(item.ratingRevievs.replace(/\D/g, ""));
  });
  ...
  temp.forEach((item, i) => {
    let textNodes = `
      ...
      <div class="revievs">Отзывы: ${item.ratingRevievs}</div>
    `;

    let fragment = document.createRange().createContextualFragment(textNodes);
    document.querySelector(".result").append(fragment);
  });
}

document.querySelector(".product-revievs").addEventListener("click", () => sortByFeedbacks(items));
...
document.querySelector(".product-price").addEventListener("click", () => sortByPrice(items));
